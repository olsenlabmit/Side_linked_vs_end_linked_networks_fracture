...
######## Algorithm: modified based on AA Gusev, Macromolecules, 2019, 52, 9, 3244-3251 ##########

######## Updated 2025-26: Devosmita Sen & Hari Ramakrishnan Sudhakar #############
...
##global parameters
parameters=np.zeros([3,6]) # N, b, K, fit_param, E_b,U0
...
parameters[1,:]=np.array([p.N_high,p.b_high,p.K_high,p.fit_param_high,p.E_b_high,p.U0_high])  ## this is not used anywhere- because chain types are only 0 and 2
parameters[2,:]=np.array([p.Nmain,p.b_main,p.K_main,p.fit_param_main,p.E_b_main,p.U0_main])
...
for frac_weak in frac_weak_array_py:
   ...
   import ioLAMMPS
   import netgen_sidelinked as netgen
   #from relax import Optimizer
   ...
   ##   rho  = 3
      ...
      netgen.generate_network(prob, func, parameters,L, l0, n_chains, frac_weak,m,nc,Nmain)
      directory = './'+str(int(100*frac_weak))+'/'
      filename = 'network.txt'
      file_path = os.path.join(directory, filename)
      if not os.path.isdir(directory):

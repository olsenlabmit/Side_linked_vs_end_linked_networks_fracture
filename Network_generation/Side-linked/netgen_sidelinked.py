...
# Follows algorithm modified based on
...
##### UPDATES START HERE
def generate_network(prob, func, parameters, L, l0, n_chains, frac_weak,m,nc,Nmain):
...
#m is the number of B functional groups in a single crosslinked chain
    chain_cnt=0
    primary=1
    for i in range (0, nc):
        ...
        links[i*m,:] = [x, y, z]
        conn[i*m][0]=i*m+1
        # b1: kuhn length for corsslinked chain
        # l0: kuhn length for test chains(red)
        for j in range (i*m+1, i*m+m):
            x = np.random.normal(0, math.sqrt(Nmain/3)*b1)+links[j-1,0]
            y = np.random.normal(0, math.sqrt(Nmain/3)*b1)+links[j-1,1]
            z = np.random.normal(0, math.sqrt(Nmain/3)*b1)+links[j-1,2]
            ...
            links[j,:] = [x, y, z]
            #print(i)
            chains[chain_cnt,0]=2
            chains[chain_cnt,1]=j-1
            chains[chain_cnt,2]=j
    ...
    if (primary!=0):
        for i in range(0,nc):
            chains[chain_cnt,0]=2
            chains[chain_cnt,1]=i*m
            conn[i*m][1]=n_links+1
            chain_cnt=chain_cnt+1
            chains[chain_cnt,0]=2
            chains[chain_cnt,1]=i*m+m-1
            conn[i*m+m-1][1]=n_links+1
            chain_cnt+=1
        
                
    n_onedeg=0
    chain_conn=np.zeros(nc)
    for i in range (nc*m-nc+primary*2*nc, n_chains):
...
##        N=chains[i,0]
 ...
 ########## ADDITION TO CODE
        ...
        if np.size(np.where(conn==-1))>0:
            bool=1
            #print(conn)
        else:
            bool=0
            print('no crosslink found')
        
        # Bool is a variable to check if any crosslink is even active at all instead of searhing randomly
     
        while (found == 0)  and (bool==1):
##              random.seed(10)
...
##           random.seed(10)
           ...
           if (prob > rnd_num and len(neigh) > 0):
              ...
              m_link1=link_1//m
              m_link2=link_2//m
              if (m_link1==m_link2) and ((link_1%m-link_2%m)**2==1):
                n_onedeg+=1
              elif m_link1!=m_link2:
                chain_conn[m_link1]+=1
                chain_conn[m_link2]+=1
    ...
    file1.write("Contour Length = %4.1f\n" %(N))
    ...
    file1.write('# No. of 1 degree loops = %i\n' %(n_onedeg))
    file1.write('# No of inter connected corsslink polymer chains=%i\n' %(np.size(np.where(chain_conn!=0))))
    file1.write('# dimensionless local concentration=%i\n' %(n_chains/L**3*N**1.5))
    file1.close()
...
##    file2.close()
    ...
    xlo = 0; ylo = 0; zlo = 0
    xhi = L; yhi = L; zhi = L
    atom_types = 2; bond_types = 3; n_break = 0
...
##    print( ' links',links,'  new_chains', new_chains)
    ...
    #ioLAMMPS.writeLAMMPS(file_path, xlo, xhi, ylo, yhi, zlo, zhi, links, chains, atom_types, bond_types, mass, loop_atoms)

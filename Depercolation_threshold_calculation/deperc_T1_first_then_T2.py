### Calculate depercolation threshold - first break T1 chains and then T2 #####
...
##   print(chains)
   ...
   for c in chains:
      ...
      G.add_edge(c[2],c[3], weight=c[0]+1)
      #print(c[0],c[1])
   #stop
   '''
...
directory_path='./deperc_T1_first_then_T2'
...
##               all_edges=
               ...
               edges_with_T1=[ (u, v) for u, v, d in G_temp.edges(data=True) if d.get('weight') == T1_id ]
               edges_with_T2=[ (u, v) for u, v, d in G_temp.edges(data=True) if d.get('weight') == T2_id ]
               #print(len(edges_with_target_weight),len(G_temp.edges()))
               #print('len(edges_with_T1)',len(edges_with_T1),'len(edges_with_T2)',len(edges_with_T2))
               init_num_bonds=G_temp.number_of_edges()#len(edges_with_target_weight)#
...
####                           num_broken_bonds=num_broken_bonds+1
                  ...
                  if len(edges_with_T1)>0:  ## if T1 is available- then break T1 only
                      selected_edge = random.choice(edges_with_T1)
                      ...
                      edges_with_T1.remove(selected_edge)
                      #print(selected_edge)
####                      print("Selected Edge:", selected_edge)
                      G_temp.remove_edge(selected_edge[0], selected_edge[1])  ##(node_1,node_2)
                      num_broken_bonds=num_broken_bonds+1
                      num_T1_broken=num_T1_broken+1
                      
                  elif len(edges_with_T2)>0: ## is T1 not available, but T2 available- then break T2
                      selected_edge = random.choice(edges_with_T2)
                      #idx = random.randrange(len(edges_with_target_weight))  # or np.random.randint(len(arr))
                      #selected_edge = edges_with_target_weight[idx]
                      #print('selected_edge',selected_edge)
                      #print('selected_edge[0]',selected_edge[0])
                      ...
                      edges_with_T2.remove(selected_edge)
                      #print(selected_edge)
####                      print("Selected Edge:", selected_edge)
                      G_temp.remove_edge(selected_edge[0], selected_edge[1])  ##(node_1,node_2)
                      num_broken_bonds=num_broken_bonds+1
                      num_T2_broken=num_T2_broken+1
                  else:
               ...
               #stop
               print('num_broken_bonds',num_broken_bonds, 'num_T1_broken',num_T1_broken,'num_T2_broken',num_T2_broken)
               num_T1_broken_arr.append(num_T1_broken)
               num_T2_broken_arr.append(num_T2_broken)
...
####            #print('De-gel point from second largest connected',frac_cleaved_array[i])
    ...
    np.savetxt(directory_path+'/num_broken_chains_ite_'+str(ite)+'.txt',np.transpose(np.array([frac_cleaved_array,num_T1_broken_arr,num_T2_broken_arr])))

...
def shortest_simple_paths_DS(G, source, target, max_loop_order,weight=None):
    # Updated Devosmita Sen
    # to incorporate a maximum loop order
    ...
    while True:
        if not prev_path:
            try:
                ...
                break ## DONT PUT continue!!! it will keep running forever because this is a while loop
            if(len(path)<max_loop_order):
##                if(len(path)>10): # THIS IS NOT GETTING PRINTED IN THE MULTIPROCESSED VERSION- WILL HAVE TO DEBUG IN THE NOT-MULTIPROCESSED VERSION
##                    print('in shortest_path_DS (cond 1): path is:', path)
##                    stop
            ...
            for i in range(1, len(prev_path)):
                ...
                try:
                    ...
                    if(len(path)<max_loop_order):
##                        if(len(path)>10): # THIS IS NOT GETTING PRINTED IN THE MULTIPROCESSED VERSION- WILL HAVE TO DEBUG IN THE NOT-MULTIPROCESSED VERSION
##                            print('in shortest_path_DS (cond 2): path is:', path)
##                            print('len(path)=',len(path))
##                            print('source',source)
##                            print('target',target)##                            stop
...
def _bidirectional_shortest_path(
    ...
    except nx.NetworkXNoPath: ## added by DS
